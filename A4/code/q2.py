...
def q1(bn):
    ...
    n0 = VE(bn, asys, [af]) #    #v1 = air system, v2= car cranks, v3 =air filter
    # pv.set_evidence('strong')
    ...
    n3 = VE(bn, asys, [af, cc]) # v3 = battery voltage
...
def q2(bn):
    ...
    n3 = VE(bn, bv, [pv, mf])  # v1 = battery age, v2 = charging system, v3 = battery voltage
    print(n1[0], '<', a1[0])
    print(n3[0], '<', a3[0])
...
def q3(bn):
    ...
    pv.set_evidence('strong') # voltage at plug, strong

    n0 = VE(bn, pv, [])

    ds.set_evidence('okay')  # distributer, okay
    n1 = VE(bn, pv, [ds])

    mf.set_evidence('okay') # main fuse, okay
    n2 = VE(bn, pv, [ds, mf])

    ba.set_evidence('new')
    n3 = VE(bn, pv, [ds, mf, ba]) # battery age, new

    cs.set_evidence('okay')
    n4 = VE(bn, pv, [ds, mf, ba, cs]) # charging system, okay

    st.set_evidence('true')
    n5 = VE(bn, pv, [ds, mf, ba, cs, st]) # car starts, strong
...
def q4(bn):
    ...
    sm.set_evidence('faulty') # starter motor, faulty
    n1 = VE(bn, pv, [sm])

    st.set_evidence('false') # car starts, false
    n2 = VE(bn, pv, [ds, sm, st])

    ba.set_evidence('old')
    n3 = VE(bn, pv, [ds, sm, st, ba])  #battery age, old

    cc.set_evidence('true')
    n4 = VE(bn, pv, [ds, sm, st, ba, cc]) # car cranks, true

    bv.set_evidence('weak')
    n5 = VE(bn, pv, [ds, sm, st, ba, cc, bv])  # batteru voltage, weak
